...
class PyTorchWorker(Worker):
    def __init__(self, N_train = 8192, N_valid = 1024, **kwargs):
            ...
            self.train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, sampler=train_sampler, **kwargs)
            self.validation_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=1024, sampler=validation_sampler, **kwargs)

            self.test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=1024, shuffle=False, **kwargs)
...
if __name__ == "__main__":
    ...
    config = {
                'dropout_rate': 0.2238820806451817,
                'lr': 0.000263874483466196,
                'num_conv_layers': 2, 
                'num_fc_units': 105, 
                'num_filters_1': 28, 
                'optimizer': 'Adam', 
...
#     print(config)
    res = worker.compute(config=config, budget=234, working_directory='.')
    print(res["info"])